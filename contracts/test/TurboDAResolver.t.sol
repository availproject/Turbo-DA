...
contract TurboDAResolverTest is Test {
    ...
    function setUp() public {
        ...
        depositContract.configureSigner(user1);
    ...
    function testETHWithdrawal() public {
        // Prepare user ID and deposit
        bytes memory userID = abi.encodePacked("testUser");
        uint256 depositAmount = 10 ether;

        // Deposit ETH
        vm.prank(user1);
        vm.deal(user1, depositAmount);
        depositContract.deposit{value: depositAmount}(userID);
        ...
        // Perform withdrawal
        vm.prank(user1);
        depositContract.withdrawWithSignature(
            userID,
            address(0),
            depositAmount,
            user1,
        ...
        assertEq(
            ...
            "Recipient should receive the correct amount of ETH"
        );
        assertEq(
            depositContract.userDeposits(userID, address(0), user1),
            0,
            "User deposit should be zero after withdrawal"
        );
    }
    ...
    function testERC20Withdrawal() public {
        ...
        mockToken.approve(address(depositContract), depositAmount);
        vm.prank(user1);
        depositContract.depositERC20(userID, depositAmount, address(mockToken));
        ...
        assertEq(
            depositContract.userDeposits(userID, address(mockToken), user1),
            0,
            "User deposit should be zero after withdrawal"
        );
    }

    // Test withdrawal with invalid signature
    function testRevertOnInvalidSignature() public {
        ...
        bytes memory invalidSignature = _createSignature(
            userID,
            address(0),
            depositAmount,
            user2
    ...
    // Test withdrawal with insufficient deposit
    function testRevertOnInsufficientDeposit() public {
        // Prepare user ID and deposit
        bytes memory userID = abi.encodePacked("testUser");
        uint256 depositAmount = 10 ether;
        ...
        vm.prank(user1);
        vm.expectRevert(TurboDAResolver.InsufficientDeposit.selector);
        depositContract.withdrawWithSignature(
            userID,
            address(0),
            withdrawAmount,
