...
use turbo_da_core::utils::{
    format_size, generate_submission_id, get_connection, retrieve_encrypted_data, retrieve_user_id,
};
...
pub async fn submit_data(
    ...
    let encrypted_data = match retrieve_encrypted_data(&http_request) {
        Some(val) => val,
        None => {
            return HttpResponse::InternalServerError()
                .json(json!({ "error": "Encrypted data not retrieved" }))
        }
    };
